            ...
            <div class="container-fluid">
                <div class="row">
                    <ol class="breadcrumb float-sm-left pl-3">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('page-admin-product-list') }}" title="Quản lý hoá đơn">Quản lý
                                hoá đơn</a></li>
                ...
                <form class="validation-form" method="post" action="" enctype="multipart/form-data">
                    <div class="card-footer text-sm sticky-top">
                        ...
                        <button type="reset" class="btn btn-sm bg-gradient-secondary"><i class="fas fa-redo mr-2"></i>Làm
                            lại</button>
                        <a class="btn btn-sm bg-gradient-danger" href="{{ route('page-admin-order-list') }}" title="Thoát"><i
                            ...
                            <div class="card card-default color-palette-box card-primary card-outline text-sm">
                                ...
                                <div class="card-body row">
                                    ...
                                    <div class="form-group title col-md-12 col-sm-12">
                                        ...
                                        <select id="select-status" name="status" class="form-control select2" {{ $orderDetail->status == 'completed' ? 'disabled' : '' }}>
                                            <option value="cancelled"{{ ($orderDetail->status=='cancelled')? "selected" : "" }}>Đã Hủy</option>
                                            <option value="pending" {{ ($orderDetail->status=='pending')? "selected" : "" }}>Mới Đặt</option>
                                            <option value="paid"{{ ($orderDetail->status=='paid')? "selected" : "" }}>Đã Thanh Toán</option>
                                            <option value="delivering"{{ ($orderDetail->status=='delivering')? "selected" : "" }}>Đang giao</option>
                                            <option value="completed"{{ ($orderDetail->status=='completed')? "selected" : "" }}>Hoàn tất</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                                                    ...
                                                    <tr data-id="{{ $item->id }}">
                                                        ...
                                                        <td class="align-middle">
                                                            <a class="text-dark text-break"
                                                                href="{{ route('page-admin-product-update', ['id' => $item->id_product]) }}"
