...
/* Owl Data */
InfinityShop.OwlData = function (obj) {
...
/* Owl Page slide */
InfinityShop.OwlPage = function () {
    if ($(".owl-page").length > 0) {
        $(".owl-page").each(function () {
            InfinityShop.OwlData($(this));
        });
    }
...
InfinityShop.Cart = function () {
    ...
    $("body").on("click", ".quantity-counter-procart span", function () {
        ...
        input.val(oldValue);
        updateCart(id, oldValue);
    });

    function updateCart(id = 0, quantity = 1) {
        if (id) {
...
InfinityShop.Search = function () {
    ...
    $("body").on("click", ".btn-search", function () {
        if ($("#keyword").val() == "") {
            ...
            window.location.href = "/product/search/" + $("#keyword").val();
        }
    });
};

InfinityShop.RenderPicture = function () {
    /* Reader image */
...
InfinityShop.CheckSubmit = function () {
    $("body").on("submit", "#form-cart", function (e) {
        e.preventDefault();
        var selectedPaymentMethod = $(".payment-method:checked").val();
        ...
        if (selectedPaymentMethod === "cod") {
            $.ajax({
                ...
                success: function () {
                    ...
                    // }, 3000);
                    // location.reload();
                    window.location.href = "/user/purchase-history";
                    return false;
                },
            });
...
InfinityShop.ShowPassword = function () {
    /* Show old password */
    $(".show").click(function () {
        if ($("#old-password").val()) {
...
$(document).ready(function () {
    InfinityShop.Menu();
    InfinityShop.OwlPage();
    InfinityShop.Cart();
    InfinityShop.Search();
    InfinityShop.RenderPicture();
    InfinityShop.CheckSubmit();
    InfinityShop.ShowPassword();
});
