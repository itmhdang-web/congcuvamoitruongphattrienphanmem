...
class HomeController extends Controller {
    ...
    public function userIndexPage(Request $req) {
        $dsProductNew = TableProduct::where('deleted_at', null)
            ->orderBy("created_at", "desc")
            ->get();

        $dsProductOutStanding = TableProduct::where('deleted_at', null)
            ->orderBy("view", "desc")
            ->get();

        $dsProductDiscount = TableProduct::where('deleted_at', null)
            ->where('sale_price', '>', 0)
            ->orderBy("created_at", "desc")
            ->get();

        return view('.user.home.home', compact('dsProductNew', 'dsProductOutStanding', 'dsProductDiscount'));
    }
